    ...
    if keys != []:
        if 'z' in keys:
            results.append({'color': tuple(element_master.color),
                            'seen': 1})
            hue = np.random.randn() * 0.01 - 1.4 
            value = np.random.randn() * 0.05 + 0.5
            saturation = np.random.randn() * 0.1 + 0.3
        if 'm' in keys:
            results.append({'color': tuple(element_master.color),
                            'seen': 0})
            hue += np.random.randn() * 0.01
            value += np.random.randn() * 0.01 + 0.01
            saturation += np.random.randn() * 0.01 - 0.01
        if 'n' in keys:
            results.append({'color': tuple(element_master.color),
                            'seen': 0.5})
            hue += np.random.randn() * 0.01
            value += np.random.randn() * 0.01
            saturation += np.random.randn() * 0.01

        element_master.color = [hue, saturation, value]

        if 'q' in keys:
            pandas.DataFrame(results).to_csv('results.csv')
            break
